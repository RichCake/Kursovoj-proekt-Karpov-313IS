...
class AcceptRequestRegistry(QWidget):
    def __init__(self, parent):
        ...
        cur = con.cursor()
        request_ids = cur.execute("SELECT DISTINCT request_id FROM Request_approvals_stages WHERE acceptor_id=? AND approval_status='Не согласовано';", (self.parent.user_id,)).fetchall()
        con.close()
    ...
    def accept_request(self):
        ...
        cur.execute("UPDATE Request_approvals_stages SET comment=?, approval_status=? WHERE acceptor_id=? AND request_id=?;", (comment, "Согласовано", self.parent.user_id, request_id))
        con.commit()
        con.close()
        self.refresh_list()
    ...
    def reject_request(self):
        ...
        print(request_id)
        print(self.parent.user_id)
        cur.execute("UPDATE Request_approvals_stages SET comment=?, approval_status=? WHERE acceptor_id=? AND request_id=?;", (comment, "Отклонено", self.parent.user_id, request_id))
        con.commit()
        con.close()
        self.refresh_list()
        self.check_to_change_request_status()
