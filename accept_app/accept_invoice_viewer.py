...
class AcceptInvoiceWidget(QWidget):
    def __init__(self, parent, invoice_id):
        ...
        self.model.setTable("Invoice_approvals_stages")
        ...
        self.model.setFilter(f"invoice_id = {self.invoice_id}")
        self.model.select()
    ...
    def add_accept_row(self):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            max_stage = cur.execute("SELECT MAX(stage_order) FROM Invoice_approvals_stages WHERE invoice_id=?;", (self.invoice_id,)).fetchone()[0]
            if not max_stage:
                max_stage = 0
            else:
            ...
            for stage_order, acceptor_id in enumerate(dialog.accepted_users):
                cur.execute("""
                            ...
                            VALUES (?, ?, ?, ?)
                            """, (approval_status, max_stage + stage_order if dialog.is_step_by_step else 1, self.invoice_id, acceptor_id))
